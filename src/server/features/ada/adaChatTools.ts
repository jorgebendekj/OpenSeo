...
// Ada reads more of a site than the onboarding preview: enough pages to work
// out what a business does, sells, and positions against on its own.
const Ada_MAX_SCRAPE_PAGES = 10;
const Ada_MAX_MAPPED_URLS = 60;

// Shape of the MCP tool objects exported from src/server/mcp/tools/*. Ada reuses
...
// Adapt one Findable tool into an AI SDK tool. The shared handler receives the
...
// external MCP path.
//
// Ada always runs inside one project (the session row), so we bind that project
...
// the session history. Ada's get_audit_status therefore waits server-side:
  ...
  return {
    map_links: tool({
      ...
      execute: async ({ domain }) => {
        ...
        const result = await discoverSiteUrls(target, Ada_MAX_MAPPED_URLS);
    ...
    read_pages: tool({
      description: `Read up to ${Ada_MAX_SCRAPE_PAGES} web pages as plain text — the project's own pages or anyone else's (competitors, references). Pass specific \`urls\` (usually picked from map_links); omit to read a representative sample of the project's own site. Uses no credits.`,
      inputSchema: z.object({
        urls: z
          .array(z.string().url())
          .max(Ada_MAX_SCRAPE_PAGES)
          .optional()
          .describe(
            `Specific page URLs to read (max ${Ada_MAX_SCRAPE_PAGES}). Omit to read the project's own site.`,
          ),
      }),
      execute: async ({ urls }) => {
        const site =
          urls && urls.length > 0
            ? await readPages(urls, Ada_MAX_SCRAPE_PAGES)
            : projectDomain
              ? await readSite(projectDomain, Ada_MAX_SCRAPE_PAGES)
              : null;
        if (!site) {
          return {
            error:
...
/**
 * Builds Ada's tool surface as an AI SDK ToolSet: the full MCP toolset plus the
 * free site-reading tools. Every tool the Findable MCP server exposes is
 ...
 * anyone noticed.
 */
export function buildAdaMcpTools(
  ...
  // Note: no `list_projects`. Ada is bound to the session's project, so
  ...
  return {
    ...
    get_product_info: tool({
      description:
        "The Findable fact sheet: what the product does, plans/pricing, credit costs, integrations, MCP setup. Call before answering questions about Findable itself. Uses no credits.",
      inputSchema: z.object({}),
      execute: () => Promise.resolve({ factSheet: findableFactSheet }),
    }),
...
export const buildSamMcpTools = buildAdaMcpTools;
