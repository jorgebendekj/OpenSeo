...
import { AdaSessionRepository } from "@/server/features/ada/AdaSessionRepository";
import { ProjectContextService } from "@/server/features/project-context/services/ProjectContextService";
import { ProjectRepository } from "@/server/features/projects/repositories/ProjectRepository";
import { buildAdaMcpTools } from "@/server/features/ada/adaChatTools";
import { buildAdaSkillSource } from "@/server/features/ada/adaSkills";
import { buildAdaSystemPrompt } from "@/server/features/ada/adaSystemPrompt";
...
// Ada's read-only view of the project's shared memory. The block has no `set`
...
type AdaContext = {
  row: NonNullable<
    Awaited<ReturnType<typeof AdaSessionRepository.getSessionById>>
  >;
  project: NonNullable<
...
/**
 * Durable Object backing the Ada in-app agent, built on Think. One DO per chat
 ...
 */
export class AdaChatAgent extends Think {
  // Ada's toolset is the MCP tools from beforeTurn; it has no use for Think's
  ...
  private adaContext: AdaContext | null = null;
  ...
  getModel() {
    ...
    if (!apiKey) {
      throw new Error("OPENROUTER_API_KEY is required for the Ada agent");
    }
  ...
  override afterToolCall(ctx: ToolCallResultContext) {
    ...
    this.ctx.waitUntil(
      captureServerEvent({
        distinctId: this.adaContext.row.userId,
        event: "sam:skill_activated",
        organizationId: this.adaContext.project.organizationId,
        properties: {
          skill,
          success: ctx.success,
          duration_ms: ctx.durationMs,
          project_id: this.adaContext.project.id,
          source: "in_app_agent",
        },
      }),
    );
  ...
  private async loadAdaContext(): Promise<AdaContext | null> {
    if (this.adaContext) return this.adaContext;
    const row = await AdaSessionRepository.getSessionById(this.name);
    if (!row) return null;
    ...
    this.adaContext = { row, project, userEmail: creator.email };
    return this.adaContext;
  }
  ...
  private buildSoulPrompt(): Promise<string> {
    return withPgClient(async () => {
      const ctx = await this.loadAdaContext();
      if (!ctx) {
        return "You are Ada, the SEO agent inside Findable. This chat session no longer exists; tell the user to start a new chat.";
      }
      const context = await ProjectContextService.getProjectContext(
        ctx.project.id,
      );
      return buildAdaSystemPrompt(
        {
        ...
        // Nothing recorded about the business yet: Ada runs its intake flow.
  ...
  async beforeTurn(_ctx: TurnContext): Promise<TurnConfig> {
    ...
    return withPgClient(async (): Promise<TurnConfig> => {
      ...
      // Gate every turn on credits in hosted mode: Ada is open to every plan
      ...
      if (await isHostedServerAuthMode()) {
        ...
        if (depleted) {
          return this.refusalTurn(
            "You're out of credits. Top up to keep using Ada.",
          );
        }
      ...
      return {
        tools: buildAdaMcpTools(authContext, {
          id: ctx.project.id,
          domain: ctx.project.domain,
        }),
        // Ada is meant to run complex multi-step work in one turn (site-read
  ...
  async onChatResponse(result: ChatResponseResult): Promise<void> {
    await withPgClient(async () => {
      ...
      if (ctx.row.title === "New chat") {
        ...
        if (title !== "New chat") {
          await AdaSessionRepository.setTitle(ctx.row.id, title);
          ctx.row.title = title;
        }
      } else {
        await AdaSessionRepository.touch(ctx.row.id);
      }
    });
...
export const SamChatAgent = AdaChatAgent;
