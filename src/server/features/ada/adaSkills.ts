...
// Bundle the repo's public-facing skills (.agents/skills) into Ada at build
...
// note reframes the surface so Ada skips the steps that don't apply in-app.
const ADA_SURFACE_NOTE = `> Surface note: you are Ada, running inside the Findable app. You are already
...
> improvising. Keep Ada's chat voice: a skill's output format is a
> checklist of what to cover, not a document template to fill.`;

type AdaSkill = { name: string; description: string; body: string };
...
function parseSkill(path: string, raw: string): AdaSkill | null {
  const match = /^---\n([\s\S]*?)\n---\n?([\s\S]*)$/.exec(raw);
  ...
  // GitHub issues for contributors, which Ada has no surface for.
  if (frontmatter.name === "simple-issue-description") return null;
  return {
    name: frontmatter.name,
    description: frontmatter.description,
    body: `${ADA_SURFACE_NOTE}\n\n${match[2].trim()}`,
  };
}
...
function fingerprint(skills: AdaSkill[]): string {
  let hash = 5381;
...
export function buildAdaSkillSource(): SkillSource {
  if (cachedSource) return cachedSource;
  const skills = Object.entries(skillFiles)
    .map(([path, raw]) => parseSkill(path, raw))
    .filter((skill): skill is AdaSkill => skill !== null)
    .toSorted((a, b) => a.name.localeCompare(b.name));

  return (cachedSource = {
    id: "findable-public-skills",
