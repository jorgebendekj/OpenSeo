...
export const Route = createFileRoute("/_marketing/backlink-checker")({
  head: () =>
    buildPageSeo({
      ...
      imageAlt: "Findable free backlink checker",
    }),
...
const FAQS = [
  {
    ...
    answer:
      "Results come from DataForSEO's link index, the same data source that powers backlink research inside Findable. The index is refreshed continuously, so counts can differ slightly from other tools that crawl the web on their own schedule.",
  },
  {
    question: "How many backlinks can I see for free?",
    answer:
      "The free checker shows a domain's summary metrics and its top 15 backlinks, one per referring domain, ranked by domain strength. Sign up for Findable to page through the full list, see referring domains and anchors, filter out spam, and export the data.",
  },
  {
    ...
    <article className="mx-auto max-w-5xl">
      ...
      <section className="mt-12 rounded-xl border border-[var(--color-border-subtle)] bg-white p-6 md:p-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--color-brand-muted)]">
          Findable puts full backlink analysis next to keyword research, rank
